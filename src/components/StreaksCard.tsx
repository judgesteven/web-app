...
const StreaksCard: React.FC<StreaksCardProps> = ({ playerId, accountName, apiKey, onEventCompleted }) => {
  ...
  useEffect(() => {
    const fetchStreaks = async () => {
      ...
      try {
        ...
        const data = await response.json()
        console.log('Parsed streaks data:', JSON.stringify(data, null, 2))

        // Process started streaks
        const activeStreaks = await Promise.all(
          (data.streaks?.started || []).map(async (streak: any) => {
        ...
        // Filter out any null streaks (where we couldn't get event IDs)
        const validStreaks = activeStreaks.filter((streak): streak is Streak => streak !== null)
        console.log('Final active streaks with event IDs:', JSON.stringify(validStreaks, null, 2))
  ...
  console.log('Rendering StreaksCard with:', {
    isLoading,
    streaksCount: streaks.length,
    streaks
  })

  // Always render the card, even if empty, to help with debugging
  return (
      ...
      <div className="w-full p-4 bg-white/80 backdrop-blur-xl rounded-2xl shadow-lg border border-gray-100/50">
        {isLoading ? (
          ...
          <div className="space-y-4">
            {streaks.map((streak) => {
              const isCompletingStreak = isCompleting.has(streak.id)
              const hasEventId = !!streak.eventId
                  ...
                  <div className="flex flex-col gap-2">
                    <div className="flex justify-between items-start">
                      ...
                      <button
                        ...
                        disabled={isCompletingStreak || !hasEventId}
